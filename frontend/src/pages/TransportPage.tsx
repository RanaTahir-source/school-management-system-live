...
import { Bus, Plus, Route as RouteIcon, Trash2, User as UserIcon, UserRoundCog } from 'lucide-react';
...
import type { School, StudentProfile, Driver, Vehicle, TransportRoute, StudentTransportAssignment } from '@/types';
  ...
  return (
    <div className="space-y-5">
      ...
      <Dialog open={driverOpen} onOpenChange={setDriverOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Drivers don't need a login account - just contact details for reference.</DialogDescription>
          </DialogHeader>
          <form onSubmit={submitDriver} className="space-y-4">
            {isUnrestricted && (
              <Field label="School" required>
                <Select value={driverForm.schoolId} onValueChange={(v) => setDriverForm((f) => ({ ...f, schoolId: v }))}>
                  <SelectTrigger>
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setDriverOpen(false)}>
                Cancel
              </Button>
      ...
      <Dialog open={vehicleOpen} onOpenChange={setVehicleOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Register a van, bus, or coaster used for student transport.</DialogDescription>
          </DialogHeader>
          <form onSubmit={submitVehicle} className="space-y-4">
            {isUnrestricted && (
              <Field label="School" required>
                <Select value={vehicleForm.schoolId} onValueChange={(v) => setVehicleForm((f) => ({ ...f, schoolId: v }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select school" />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <Dialog open={routeOpen} onOpenChange={setRouteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create a pickup route, then add stops to it.</DialogDescription>
          </DialogHeader>
          <form onSubmit={submitRoute} className="space-y-4">
            {isUnrestricted && (
              <Field label="School" required>
                <Select value={routeForm.schoolId} onValueChange={(v) => setRouteForm((f) => ({ ...f, schoolId: v }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select school" />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <DialogFooter>
              ...
              <Button type="submit" loading={createRoute.isPending}>
                Add Route
              </Button>
            </DialogFooter>
          </form>
      ...
      <Dialog open={stopOpen} onOpenChange={setStopOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Pickup Stop</DialogTitle>
            <DialogDescription>Add a boarding point to this route.</DialogDescription>
          </DialogHeader>
          <form onSubmit={submitStop} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" loading={createStop.isPending}>
                Add Stop
              </Button>
            </DialogFooter>
          </form>
